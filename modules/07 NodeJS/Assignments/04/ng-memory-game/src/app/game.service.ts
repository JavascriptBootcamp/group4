...
export class GameService {
  ...
  initAvailableCards() {
    ...
    // ]
    this.replaceCardsImages('animals');
  ...
  //my add
  async replaceCardsImages(searchStr) {
    this.availableCards = [];
    const imgLst = await fetch(`http://localhost:8000?search=${searchStr}`)
      .then(response => response.json());

    for(let i=1; i<imgLst.length; i++){
      this.availableCards.push(
        { content: imgLst[i], selected: false, correct: false  }
      )
    }
    this.shuffle();
  };
